...
Download all files from this branch into one folder, then run:

```sh
cat "Moondew Canyon.exe.part"* > "Moondew Canyon.exe"
cat "Moondew Canyon.pck.part"* > "Moondew Canyon.pck"
```

Keep the rebuilt `.exe` and `.pck` in the same folder, then run `Moondew Canyon.exe`.
